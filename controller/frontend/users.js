...
async function updateUserMessage(ctx, next) {
    const user_id = ctx._token._id;
    ...
    let user_last_login_time = Date.parse(Date('Y-m-d'));
    let obj = {
        'user_name': user_name,
        'user_password': user_password,
        'user_email': user_email,
    ...
    let ress
    try {
        let ress = await users.update({ _id: user_id }, obj)
        ctx.status = 200;
        return ctx.body = {
            success: true,
            loginname: ress.user_name,
